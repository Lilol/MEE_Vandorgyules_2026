...
\node[rfbox] (rf) at (4.5, 0) {%
  {\scriptsize\bfseries Véletlen erdő\\regresszió}\\[5pt]
  {\tiny nemlineáris\\fakegyüttes-modell}
};

%% Box 3: Target variables
\node[targbox] (targ) at (8.2, 0) {%
  {\scriptsize\bfseries\color{mediumblue2}Célváltozók}\\[8pt]
  {\normalsize\bfseries\color{mediumblue}SCI}\\[4pt]
  {\normalsize\bfseries\color{mediumblue}SSI}\\[4pt]
  {\normalsize\bfseries\color{mediumblue}SP}
};

%% Box 4a: Gini importance
\node[metbox] (gini) at (12.1, 1.15) {%
  {\scriptsize\bfseries\color{mediumblue2}Gini (MDI)}\\[3pt]
...
\node[metbox] (drop) at (12.1, -1.15) {%
  {\scriptsize\bfseries\color{mediumblue2}Drop-column}\\[3pt]
...
\node[card1] at (0, 0) {%
  ...
  {\scriptsize\color{lightblue1}jellemzőfontosság és\\gazdasági hatás alapján\\egyaránt a legerősebb DSM-elem}%
};
%% Card 2 – Kombináció → nettó nulla
\node[card2] at (6.5, 0) {%
  {\huge\bfseries\color{yellow2}2}\\[6pt]
  {\normalsize\bfseries\color{mediumblue2}Kombináció $\rightarrow$\\nettó nulla számla}\\[10pt]
  {\small\bfseries\color{mediumblue}PV + DSM + Akkumulátor}\\[8pt]
  {\scriptsize\color{mediumblue2}szaldós villamosenergia-kiadás\\optimális beállítással\\közel nullára csökken}%
};
\end{tikzpicture}
\end{center}
\end{frame}
