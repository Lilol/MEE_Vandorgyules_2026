...
\section{Bevezetés}
\begin{frame}{Motiváció és háttér}
\begin{itemize}
    \item \gn{Megújulók terjedése}: 2024-re a beépített PV kapacitás $>$ \SI{6000}{MWp}, HMKE $>$ \SI{2300}{MWp}
    \item A háztartások energiaigénye (pl.~esti csúcs) jellemzően nem esik egybe a napközbeni termeléssel.  
    \item \gn{KIF hálózati problémák}: feszültségemelkedés, aszimmetria, frekvenciaproblémák, trafó túlterhelődés  
    \item \gn{Lehetséges megoldások}: 
    \begin{itemize}
        \item akkumulátoros energiatárolás,  
        \item keresletoldali szabályozás -- vezérelt terhelések, HMV, hőszivattyúk
    \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Célkitűzés}
