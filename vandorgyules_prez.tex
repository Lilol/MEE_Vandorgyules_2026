...
\hspace{-1em}\begin{minipage}{0.27\columnwidth}
\begin{itemize}\setlength{\itemsep}{1.2ex}
    \item Magyar, smart metered, éves
    \item \highlight{„Shift potential"}: az átütemezhető fogyasztás mennyisége
    \item \highlight{min}, \highlight{max}, \highlight{átl}
    \item \highlight{6 kiindulási profil}
\end{itemize}
\end{minipage}
\hspace{0.1em}
...
\section{Eredmények}
\begin{frame}{A DSM technikák hatásosságának elemzése}
\vspace{-0.5em}
\begin{center}
\scalebox{0.87}{%
\begin{tikzpicture}[x=1cm, y=1cm,
  featbox/.style={rectangle, rounded corners=3pt, fill=lightblue1, draw=mediumblue2,
                  line width=0.7pt, text width=2.9cm, minimum height=4.0cm,
                  align=left, inner sep=6pt},
  rfbox/.style={rectangle, rounded corners=3pt, fill=mediumblue, draw=mediumblue2,
                line width=0.7pt, text width=2.5cm, minimum height=2.8cm,
                align=center, inner sep=6pt, text=white},
  targbox/.style={rectangle, rounded corners=3pt, fill=lightblue1, draw=mediumblue2,
                  line width=0.7pt, text width=2.0cm, minimum height=2.8cm,
                  align=center, inner sep=6pt},
  metbox/.style={rectangle, rounded corners=3pt, fill=yellow1, draw=yellow2,
                 line width=0.7pt, text width=3.3cm, minimum height=1.8cm,
                 align=left, inner sep=6pt},
  resbar/.style={rectangle, fill=mediumblue2, text=white,
                 text width=13.2cm, minimum height=0.85cm, align=center,
                 font=\bfseries},
  arr/.style={->, >=stealth, line width=0.9pt, color=mediumblue2}
]

%% Box 1: Input features
\node[featbox] (feat) at (0, 0) {%
  {\scriptsize\bfseries\color{mediumblue2}Bemeneti jellemzők}\\[5pt]
  {\tiny
  $\bullet$~Alapprofil típusa\\[2pt]
  $\bullet$~Háztartási berende-\\
  \hspace*{5pt}zések (igen/nem)\\[2pt]
  $\bullet$~Épületgépészet,\\
  \hspace*{5pt}HMV-fogyasztás\\[2pt]
  $\bullet$~Akkumulátorméret\\
  \hspace*{5pt}[kWh/kWp]\\[2pt]
  $\bullet$~PV-tájolás (DK/DNy)}
};

%% Box 2: RF model
\node[rfbox] (rf) at (4.5, 0) {%
  {\scriptsize\bfseries Random forest\\regresszió}\\[5pt]
  {\tiny nemlineáris modell}
};

%% Box 3: Target variables
\node[targbox] (targ) at (8.2, 0) {%
  {\scriptsize\bfseries\color{mediumblue2}Célváltozók}\\[8pt]
  {\normalsize\bfseries\color{mediumblue}SC}\\[2pt]
  {\normalsize\bfseries\color{mediumblue}SS}\\[2pt]
  {\normalsize\bfseries\color{mediumblue}SP}\\[2pt]
  {\normalsize\bfseries\color{mediumblue}GL}
};

%% Box 4a: Gini importance
\node[metbox] (gini) at (12.1, 1.15) {%
  {\scriptsize\bfseries\color{mediumblue2}\textit{Gini} fontosság}\\[3pt]
  {\tiny fa-alapú szórás-\\csökkentés\\[2pt]%
  \textit{gyors, de torzíthat}}
};

%% Box 4b: Drop-column importance
\node[metbox] (drop) at (12.1, -1.15) {%
  {\scriptsize\bfseries\color{mediumblue2}\textit{Drop-column} fontosság}\\[3pt]
  {\tiny $R^2$ csökkenése\\[2pt]%
  \textit{torzítatlan,\\számításigényes}}
};

%% Arrows
\draw[arr] (feat.east) -- (rf.west);
\draw[arr] (rf.east) -- (targ.west);
\draw[arr] (targ.east) -- +(0.45,0) |- (gini.west);
\draw[arr] (targ.east) -- +(0.45,0) |- (drop.west);

%% Bottom bar
\node[resbar] at (5.85, -2.6) {1\,152\,db szimuláció};
...
\begin{frame}{DSM technikák hatásossága}
\centering
\includegraphics[width=0.75\columnwidth]{figures/fig_3_1_fontossag_akkumulator_nelkul.png}
\end{frame}


\begin{frame}{Akkumulátor méretének hatása}
\centering
\includegraphics[width=\columnwidth]{figures/fig_3_2_fontossag_bess_meret.png}
\end{frame}

\begin{frame}{Akkumulátor kölcsönhatások}
...
\begin{center}
\begin{tikzpicture}[x=1cm, y=1cm,
  card1/.style={rectangle, rounded corners=6pt, fill=mediumblue, draw=mediumblue2,
                line width=1.2pt, text width=5.2cm, minimum height=5.2cm,
                align=center, inner sep=14pt, text=white},
  card2/.style={rectangle, rounded corners=6pt, fill=yellow1, draw=yellow2,
                line width=1.2pt, text width=5.2cm, minimum height=5.2cm,
                align=center, inner sep=14pt},
]
%% Card 1 – Az akkumulátor dominál
\node[card1] at (0, 0) {%
  {\huge\bfseries\color{lightblue1}1}\\[6pt]
  {\normalsize\bfseries Az akkumulátor\\a meghatározó}\\[10pt]
  {\small\bfseries\color{lightblue1}SC\,+40\% \enspace|\enspace ${\sim}$170\,000\,Ft/év}\\[8pt]
  {\scriptsize\color{lightblue1}jellemzőfontosság és\\gazdasági hatás alapján\\a leghatásosabb DSM technika}%
};
%% Card 2 – Kombináció → nettó nulla
\node[card2] at (6.5, 0) {%
  {\huge\bfseries\color{yellow2}2}\\[6pt]
  {\normalsize\bfseries\color{mediumblue2}Az átütemezhetőség megmarad $\rightarrow$\\az akkumulátor mellett is}\\[10pt]
  {\small\bfseries\color{mediumblue}PV + DSM + Akkumulátor}\\[8pt]
  {\scriptsize\color{mediumblue2}erős kölcsönhatás akkumulátor és eltolási potenciálok között}%
};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{1.2ex}
    \item Adatvezérelt elemzés: \highlight{akkumulátorméret} és \highlight{PV-tájolás}
          a leghangosabb jellemzők SC/SS/SP szempontjából
    \item Az akkumulátor önmagában \highlight{SC\,+40\%}
          és \highlight{${\sim}$170\,000\,Ft/év} megtakarítást jelent
    \item PV\,+\,rugalmas DSM\,+\,akkumulátor kombinációval a nettó
          villamosenergia-kiadás \highlight{közel nullára} csökken
\end{itemize}
\end{minipage}
\hspace{5em}
